...
def fit(x, y, first_point = 0, last_point = -1, f_fit = None):
    ...
    # x=(np.array(res_aver['phi']) - res_aver['phi'][0])/res_aver['phi'][0]
    # y=np.array(res_aver['r'])/res_aver['r'][0]
    popt, pcov = curve_fit(f_fit, x[first_point:last_point], y[first_point:last_point])
    def f_fit_opt(x):
        return f_fit(x,*popt)
