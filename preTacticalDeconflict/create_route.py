...
def get_originale_net(config,newEff,Nflight):
	...
	'Calculate angle between different OE'
	...
	L=[g.shortest_paths(source=to_str2(a[0]),target=to_str2(a[1]),weights=g.es["weight"])[0][0] for a in Ap]
	S=[dist([gall(a[0]),gall(a[1])]) for a in Ap]
	S=pl.mean(S)
	...
	P=[[gV[e] for e in g.get_shortest_paths(to_str2(a[0]),to=to_str2(a[1]),weights=g.es["weight"])[0]] for a in Ap]
