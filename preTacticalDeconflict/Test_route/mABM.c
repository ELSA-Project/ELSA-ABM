...
void init_output(Aircraft_t *flight,int Nflight){
	...
	wstream = fopen("DATA/m3_data.dat","w");
	if(wstream==NULL) BuG("Impossible to save output data\n");
	fclose(wstream);
...
void save_m3(Aircraft_t *flight, int Nflight,Aircraft_t *Flight){

	FILE *wstream=fopen("DATA/m3_data.dat","a");
	if(wstream==NULL) BuG("Impossible to save output data\n");
...
int _calculate_st_point(Aircraft_t *f,CONF_t conf,long double t){
	...
	//_position(f,(*f).nvp[0],conf.t_w,conf.t_i));
		//(*f).ready=0;
		//return 0;
...
//~ 
//~ int _minimum_flight_distance(long double **pos,Aircraft_t **f,int N_f,int N,TOOL_f tl,CONF_t conf){
	//~ int i,j;
	//~ long double min;
//~ 
	//~ for(i=0;i<N;i++){
		//~ if(pos[i][0]!=SAFE&&N_f>0) {
//~ 
			//~ if(((int)pos[i][2])!=((int)(*f)[0].pos[i][2])||((int) (*f)[0].pos[i][0])==SAFE) min=SAFE;
			//~ else  min=haversine_distance(pos[i],(*f)[0].pos[i]);
						//~ //printf("Lo E\t%Lf\n",min);
			//~ for(j=1;j<N_f;j++) if((*f)[j].pos[i][0]!=SAFE){
				//~ if((*f)[j].pos[i][0]!=SAFE) if( fabsl((int)pos[i][2]-((int)(*f)[j].pos[i][2]))<9.99){
...
int _minimum_flight_distance(long double **pos,Aircraft_t **f,int N_f,int N,TOOL_f tl,int *neigh,int Nneigh){
	int i,j;
	long double min;
	
	
	
	for(i=0;i<N&&Nneigh>0;i++){
		if(pos[i][0]!=SAFE&&N_f>0) {

			if(((int)pos[i][2])!=((int)(*f)[0].pos[i][2])||((int) (*f)[0].pos[i][0])==SAFE) min=SAFE;
			else  min=haversine_distance(pos[i],(*f)[0].pos[i]);
						//printf("Lo E\t%Lf\n",min);
			for(j=1;j<Nneigh;j++) if((*f)[neigh[j]].pos[i][0]!=SAFE){
				if((*f)[neigh[j]].pos[i][0]!=SAFE) if( fabsl((int)pos[i][2]-((int)(*f)[neigh[j]].pos[i][2]))<9.99){
					tl.dist[i]=haversine_distance(pos[i],(*f)[neigh[j]].pos[i]);
					//printf("Qui %Lf\n",tl.dist[i]	);
					if(tl.dist[i]<min) min=tl.dist[i];
				}
			}
	...
	if(Nneigh==0) for(i=0;i<N;i++) tl.dist[i]=SAFE;
	return 1;	
}
...
int shift(Aircraft_t f,CONF_t conf,int p,int mult){
	int sf,now,end,i;
	for(i=0;i<conf.t_w;i++) if(f.pos[i][0]!=SAFE){
		now=i;
		break;
	}
	
	for(i=now;i<conf.t_w;i++) if(f.pos[i][0]==SAFE) break;
	end=i;
	//for(i=0;i<conf.t_w;i++) printf("%.0Lf\t",f.pos[i][0]);
	//printf("Continue\n");
	int mynull;
	//scanf("%d",&mynull);
	
	//do sf=225-irand(450); while((p+sf)<0);
	int SM=(int) ((15.*mult)*60./conf.t_i);
	do sf=SM-irand(SM*2); while((p+sf)<0||(p+(end-now)+sf)>=conf.t_w);
	
	//int gap=now-p+sf,h; - - now - - p - sf -----
	int gap=now-(p+sf),h;

	//printf("sf=%d\tnow: %d\tgsp %d\n",sf,now,gap);
	//for(i=0;i<10;i++) printf("%Lf\t",f.pos[i][0]);
	//scanf("%d",&mynull);

	int count;
	if(gap>0){
		 for(h=0,count=0;h<conf.t_w;h++) if( (f).pos[h][0]!=SAFE) count++;

		 //printf("%d\t%d\n",count,conf.t_w);
		 for(i=(p+sf);i<(conf.t_w-gap);i++)for(h=0;h<4;h++)  f.pos[i][h]=f.pos[i+gap][h];
		...
		for(end=now;end<conf.t_w;end++) if(f.pos[end][0]==SAFE) break;
		end--;
		for(i=end;i>-gap;i--) for(h=0;h<4;h++) f.pos[i-gap][h]=f.pos[i][h];
	}
	//for(i=0;i<conf.t_w;i++) printf("%.0Lf\t",f.pos[i][0]);
	//printf("\n");
	for(i=0;i<conf.t_w;i++) if(f.pos[i][0]!=SAFE) break;
	//printf("> %d\t%d\n",i-p,sf);
...
int _check_safe_events(Aircraft_t **f, int N_f, SHOCK_t sh,TOOL_f tl, CONF_t conf, char *out_p){
	
	int i,j,h,unsafe,*p=ialloc_vec(N_f);
	int *c=ialloc_vec(N_f),mul;
	FILE *wstream=fopen(out_p,"w");
	for(i=0;i<N_f;i++) for(j=0;j<conf.t_w;j++) if((*f)[i].pos[j][0]!=SAFE){
		p[i]=j;
		c[i]=0;
		break;
	}
	int ev=0;
	for(i=1;i<N_f;i++){
		//for(h=p[i];(*f)[i].pos[h][0]!=SAFE;h++) printf("%d\t%Lf\n",h,(*f)[i].pos[h][0]);
		_minimum_flight_distance((*f)[i].pos,f,i,conf.t_w,tl,conf.neigh[i],conf.Nneigh[i]);
		//printf("\n\n");
		//for(h=p[i];(*f)[i].pos[h][0]!=SAFE;h++) printf("%d\t%Lf\n",h,tl.dist[h]);
	...
	long double delay;
	for(i=1;i<N_f;i++) {
		for(j=0;j<conf.t_w;j++) if((*f)[i].pos[j][0]!=SAFE) break;
		delay=(j-p[i])*conf.t_i;
		for(h=0;h<(*f)[i].n_nvp;h++) (*f)[i].time[h]=(*f)[i].time[h]+delay;
		
	}


	if(wstream==NULL) BuG("Impossible to save output");
	int H,M,S,F,T[4];
	int mynull;
	fprintf(wstream,"%d\tNfligth\n",N_f);
	for(i=0;i<N_f;i++){
		fprintf(wstream,"%d\t%d\t",(*f)[i].ID,(*f)[i].n_nvp);
		for(j=0;j<(*f)[i].n_nvp;j++) {
			time_to_int((*f)[i].time[j],T);
			fprintf(wstream,"%Lf,%Lf,%.0Lf,2014-08-08 %d:%d:%d:%d\t",(*f)[i].nvp[j][0],(*f)[i].nvp[j][1],(*f)[i].nvp[j][2],T[0],T[1],T[2],T[3]);
...
int ABM(Aircraft_t **f, int N_f, CONF_t conf, SHOCK_t sh,char *out_p){
...
// plot((*f)[i], conf);
	...
	for(i=0;i<N_f;i++) {
		//print_nvp((*f)[i]);
		_position( &((*f)[i]),(*f)[i].nvp[0],conf.t_w,conf.t_i);
		for(end=0;end<conf.t_w;end++) if((*f)[i].pos[end][0]==SAFE) break;
		end--;
		delay=(int) ((*f)[i].time[0]/conf.t_i);
		//~ printf("%d End %d\tDel %d\n",i,end,delay);
		if(end+delay>=conf.t_w){
			printf("To much\n");
			wstream=fopen(out_p,"w");
			fprintf(wstream,"BUG\n");
			fclose(wstream);
			return 0;
		}
		for(j=end;j>=0;j--) for(h=0;h<4;h++) (*f)[i].pos[j+delay][h]=(*f)[i].pos[j][h];
		for(;(j+delay)>=0;j--) for(h=0;h<4;h++) (*f)[i].pos[j+delay][h]=SAFE;
		
		
		time_to_int((*f)[i].time[0],T);
		for(j=0;j<conf.t_w;j++) if((*f)[i].pos[j][0]!=SAFE) break;
		time_to_int(j*conf.t_i,TT);
		}

	_check_safe_events(f,N_f,sh,tool,conf,out_p);
