...
int set_boundary_flag_onFlight(Aircraft_t **f,int *Nflight, CONF_t c){
	...
	for(i=0;i<*Nflight;i++) {
		...
		(*f)[i].ready=0;
		(*f)[i].nvp[0][3]=0.;
		for(j=1,N=0;j<((*f)[i].n_nvp-1);j++) (*f)[i].nvp[j][3]=1.;
		(*f)[i].nvp[j][3]=0.;
...
int get_neight(CONF_t *conf, char *neigh_f,int Nf){
	FILE *rstream=fopen(neigh_f,"r");
	if(rstream==NULL) BuG("Not found neighboors file\n");
	
	(*conf).neigh = (int**) malloc(Nf*sizeof(int*));
	if((*conf).neigh==NULL) BuG("Not enought Memory\n");
	
	(*conf).Nneigh = ialloc_vec(Nf);
	
	int i,j,h;
	char c[50000];
	for(i=0;fgets(c,50000,rstream) && i<Nf ;i++){
		(*conf).Nneigh[i]=0;
		for(j=0;c[j]!='\n'&&c[j]!='\t';){
			((*conf).Nneigh[i])++;
			for(;c[j]!='\t';j++);
		...
		if( ((*conf).Nneigh[i])==0 ) continue;
		(*conf).neigh[i] = ialloc_vec((*conf).Nneigh[i]);
		for(h=0,j=0;c[j]!='\n'&&c[j]!='\t';){
			(*conf).neigh[i][h++]=atoi(c);
			for(;c[j]!='\t';j++);
			if(c[j+1]=='\n') break;
			(*conf).neigh[i][h++]=atoi(&c[++j]);
		}		
...
int init_Sector(Aircraft_t **flight,int *Nflight,CONF_t	*config, SHOCK_t *shock,char *input_file, char *neigh_file){
	
	//get_boundary("bound_latlon.dat", config);	
	get_configuration("config.cfg", config);


	//generate_temporary_point(config);
	
	*Nflight=get_M1(input_file,flight);
	
	get_neight(config,neigh_file,*Nflight);
