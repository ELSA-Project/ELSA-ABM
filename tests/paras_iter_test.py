...
from abm_strategic.utilities import read_paras
...
paras = read_paras(paras_file='paras_test.py') # Import main parameters
...
paras['n_iter'] = n_iter
paras['par_iter'] = [tuple([tuple([float(_v) for _v in _pp])  for _pp in _p])  for _p in par_iter] # transformation in tuples, because arrays cannot be keys for dictionaries.
